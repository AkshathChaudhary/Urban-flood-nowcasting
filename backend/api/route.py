...
from backend.api.roads import get_routing_engine
...
class RouteRequest(BaseModel):
    ...
    time_horizon_min: int = Field(
        ...
        ge=0,
        le=120,
        description="Forecast horizon in minutes (0 = current, 15, 30, 60, etc.)"
    )
...
def calculate_flood_route(req: RouteRequest):
    ...
    # In production, Pair B's 2D flood depth grid is queried for time_horizon_min.
    # Currently, depth_grid can be passed if available or default dry/hotspot conditions apply.
    depth_grid = None
